...
impl TcpHandler {
    ...
    pub fn set_remote_addr(&mut self, addr: Address) { self.remote_addr = addr; }
    pub fn set_buf_size(&mut self, size: usize) { self.socket_buf_size = size; }
    pub fn set_fwd_type(&mut self, fwd_type: FwdType) { self.fwd_type = fwd_type; }
    pub fn set_bind_interface(&mut self, interface: Option<String>) { self.bind_interface = interface; }
    ...
    fn set_bind_to_device(&self, fd: libc::c_int) -> Result<(), std::io::Error> {
        ...
        if let Some(ref interface) = self.bind_interface {
            if !interface.is_empty() {
                ...
                unsafe {
                    std::ptr::copy_nonoverlapping(interface.as_ptr(), ifreq.ifr_name.as_mut_ptr() as *mut u8, len);
                }
                let ret = unsafe { libc::setsockopt(fd, libc::SOL_SOCKET, libc::SO_BINDTODEVICE, &ifreq as *const _ as *const libc::c_void, std::mem::size_of::<libc::ifreq>() as libc::socklen_t) };
    ...
    pub fn on_accept(&self, event_loop: &EventLoop, _token: Token, listener: &mut TcpListener) -> Result<(), std::io::Error> {
        ...
        unsafe {
            ...
            libc::setsockopt(fd, libc::SOL_SOCKET, libc::SO_SNDBUF, &bufsize as *const _ as *const libc::c_void, 4);
            libc::setsockopt(fd, libc::SOL_SOCKET, libc::SO_RCVBUF, &bufsize as *const _ as *const libc::c_void, 4);
        }
        ...
        let remote_fd = unsafe {
            ...
            if fd < 0 { warn!("[tcp] create remote socket failed"); drop(stream); return Ok(()); }
    ...
    /// 使用 splice 进行零拷贝转发
    #[cfg(target_os = "linux")]
    fn do_splice(&self, src_fd: RawFd, dst_fd: RawFd, pipe: &mut crate::connection::SplicePipe) -> Result<isize, i32> {
        ...
        loop {
            ...
            while pipe.pending > 0 {
                let n = unsafe { libc::splice(pipe.read_fd, std::ptr::null_mut(), dst_fd, std::ptr::null_mut(), pipe.pending, SPLICE_F_MOVE | SPLICE_F_NONBLOCK) };
                if n > 0 {
                    ...
                    if e == libc::EAGAIN || e == libc::EWOULDBLOCK { return Ok(if total > 0 { total } else { -1 }); }
                    return Err(e);
                }
            }

            // 从 src 读取到 pipe
            let n_in = unsafe { libc::splice(src_fd, std::ptr::null_mut(), pipe.write_fd, std::ptr::null_mut(), chunk_size, SPLICE_F_MOVE | SPLICE_F_NONBLOCK) };
            if n_in > 0 {
                TrafficStats::global().add_tcp_received(n_in as usize);
                // 立即发送到 dst
                let n_out = unsafe { libc::splice(pipe.read_fd, std::ptr::null_mut(), dst_fd, std::ptr::null_mut(), n_in as usize, SPLICE_F_MOVE | SPLICE_F_NONBLOCK) };
                if n_out > 0 {
                    total += n_out;
                    pipe.pending = (n_in - n_out) as usize;
                    TrafficStats::global().add_tcp_sent(n_out as usize);
                    if pipe.pending > 0 { return Ok(total); }
                } else if n_out == 0 {
                    pipe.pending = n_in as usize;
                    return Ok(total);
                } else {
                    let e = unsafe { *libc::__errno_location() };
                    if e == libc::EAGAIN || e == libc::EWOULDBLOCK { pipe.pending = n_in as usize; return Ok(if total > 0 { total } else { -1 }); }
                    return Err(e);
                }
            } else if n_in == 0 {
                return Ok(0); // EOF
    ...
    pub fn on_read(&self, event_loop: &EventLoop, _token: Token, fd64: Fd64) -> Result<(), std::io::Error> {
        ...
        if !fd_manager.exist(fd64) { return Ok(()); }
        let conn_arc = match tcp_manager.get_connection_by_any_fd(&fd64) { Some(c) => c, None => return Ok(()) };

        // 处理连接中状态
        {
            let cg = conn_arc.read().expect("poisoned");
            if fd64 == cg.remote.fd64 && cg.remote_connecting {
                let fd = match fd_manager.to_fd(fd64) { Some(f) => f, None => return Ok(()) };
        ...
        if cg.remote_connecting && is_local { return Ok(()); }

        let (my_fd64, other_fd64) = if is_local { (cg.local.fd64, cg.remote.fd64) } else { (cg.remote.fd64, cg.local.fd64) };
        let my_fd = match fd_manager.to_fd(my_fd64) { Some(f) => f, None => return Ok(()) };
        let other_fd = match fd_manager.to_fd(other_fd64) { Some(f) => f, None => return Ok(()) };
        let addr_s = cg.addr_s.clone();

        // 使用 splice
        #[cfg(target_os = "linux")]
        {
            let pipe = if is_local { &mut cg.pipe_l2r } else { &mut cg.pipe_r2l };
            if let Some(ref mut p) = pipe {
                match self.do_splice(my_fd, other_fd, p) {
                    ...
                    Ok(_) => {
                        if p.pending > 0 {
                            if let Some(tok) = token_manager.read().expect("poisoned").get_token(&other_fd64) {
                                let mut s = unsafe { TcpStream::from_raw_fd(other_fd) };
                                poll.registry().reregister(&mut s, tok, Interest::READABLE | Interest::WRITABLE).ok();
                                let _ = s.into_raw_fd();
                            }
                        }
        ...
        // Fallback: recv/send
        let data_len = if is_local { cg.local.data_len } else { cg.remote.data_len };
        if data_len != 0 { return Ok(()); }

        let (buf_ptr, buf_len) = if is_local { (cg.local.data.as_mut_ptr(), cg.local.data.len()) } else { (cg.remote.data.as_mut_ptr(), cg.remote.data.len()) };
        let recv_len = unsafe { libc::recv(my_fd, buf_ptr as *mut libc::c_void, buf_len, 0) };
        if recv_len > 0 { TrafficStats::global().add_tcp_received(recv_len as usize); }
        ...
        if is_local { cg.local.data_len = recv_len as usize; cg.local.begin = 0; }
        else { cg.remote.data_len = recv_len as usize; cg.remote.begin = 0; }

        let (send_ptr, send_len) = if is_local { (cg.local.data.as_ptr(), cg.local.data_len) } else { (cg.remote.data.as_ptr(), cg.remote.data_len) };
        ...
        if sent > 0 {
            ...
            if is_local { cg.local.data_len -= sent as usize; cg.local.begin += sent as usize; }
            else { cg.remote.data_len -= sent as usize; cg.remote.begin += sent as usize; }
        } else if sent < 0 && std::io::Error::last_os_error().kind() != std::io::ErrorKind::WouldBlock {
            #[cfg(target_os = "linux")] cg.close_pipes();
            drop(cg);
            self.close_connection(event_loop, fd64, other_fd64, my_fd, other_fd, &addr_s);
            tcp_manager.erase(&fd64);
        ...
        let pending = if is_local { cg.local.data_len } else { cg.remote.data_len };
        if pending > 0 {
    ...
    pub fn on_write(&self, event_loop: &EventLoop, _token: Token, fd64: Fd64) -> Result<(), std::io::Error> {
        ...
        let is_local = fd64 == cg.local.fd64;
        let my_fd = match fd_manager.to_fd(fd64) { Some(f) => f, None => return Ok(()) };
        let other_fd64 = if is_local { cg.remote.fd64 } else { cg.local.fd64 };
        let other_fd = match fd_manager.to_fd(other_fd64) { Some(f) => f, None => return Ok(()) };
        let addr_s = cg.addr_s.clone();
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let pipe = if is_local { &mut cg.pipe_r2l } else { &mut cg.pipe_l2r };
            if let Some(ref mut p) = pipe {
                if p.pending > 0 {
                    let n = unsafe { libc::splice(p.read_fd, std::ptr::null_mut(), my_fd, std::ptr::null_mut(), p.pending, SPLICE_F_MOVE | SPLICE_F_NONBLOCK) };
                    if n > 0 {
        ...
        let sent = unsafe { libc::send(other_fd, data_ptr.add(data_begin) as *const libc::c_void, data_len, 0) };
        if sent > 0 {
            TrafficStats::global().add_tcp_sent(sent as usize);
            if is_local { cg.local.data_len -= sent as usize; cg.local.begin += sent as usize; }
            else { cg.remote.data_len -= sent as usize; cg.remote.begin += sent as usize; }
    ...
    fn close_connection(&self, event_loop: &EventLoop, fd64: Fd64, other_fd64: Fd64, my_fd: RawFd, other_fd: RawFd, addr_s: &str) {
        let fd_manager = &event_loop.fd_manager;
        let poll = &event_loop.poll;
        let token_manager = &event_loop.token_manager;
        let tcp_manager = &event_loop.tcp_manager;
