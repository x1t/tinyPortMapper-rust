...
make test-verbose # cargo test --release -- --nocapture
...
| `CONN_CLEAR_INTERVAL_MS` | 1000ms | Connection clear interval |
...
| `DEFAULT_TCP_TIMEOUT_MS` | 360000 | TCP idle timeout |
| `DEFAULT_UDP_TIMEOUT_MS` | 180000 | UDP session timeout |
| `DEFAULT_CONN_CLEAR_RATIO` | 30 | Clear 1 connection per N |
| `DEFAULT_CONN_CLEAR_MIN` | 1 | Minimum clear count |
...
- **SO_REUSEPORT**: Multi-process binding on Linux
- **SO_BINDTODEVICE**: Interface binding support
- **IP_MTU_DISCOVER**: UDP path MTU handling
- **Signal handling**: SIGPIPE ignored, SIGTERM/SIGINT graceful exit
